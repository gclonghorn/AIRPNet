...
class LSR_Loss(nn.Module):
    ...
    def forward(self,secret_img1,secret_img2,cover_img,steg_clean2,steg_ori2,rec_img1,rec_img2,sweight1,sweight2,cweight2,
        pweight_c,finetune):
        N, _, H, W = secret_img1.size()
        out = {}
       
        lossc2 = self.mse(cover_img,steg_clean2)
        lossc2_ori = self.mse(cover_img,steg_ori2)
        losss1 = self.mse(secret_img1,rec_img1)
        losss2 = self.mse(secret_img2,rec_img2)
        percep_loss = self.vgg_loss(cover_img,steg_clean2)
        if finetune:
            loss =cweight2*lossc2+ sweight1*losss1 + sweight2*losss2 + pweight_c*percep_loss+cweight2*lossc2_ori
        else:
